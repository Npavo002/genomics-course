...
# Check to see if there are differences in the naming of countries
setdiff(world$region, daily_report_28$Country_Region) 
...
# Join the covid report with the map data
country_join <- left_join(world, daily_report_28, by = c("region" = "Country_Region"))
...
# display.brewer.all(colorblindFriendly = TRUE)
# Get and format the covid report data
report_09_28_2020 <-   read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-28-2020.csv")) %>% 
  rename(Long = "Long_") %>% 
  unite(Key, Admin2, Province_State, sep = ".") %>% 
  group_by(Key) %>% 
  summarize(Confirmed = sum(Confirmed)) %>% 
  mutate(Key = tolower(Key))
# dim(report_09_28_2020)
# get and format the map data
us <- map_data("state")
counties <- map_data("county") %>% 
  unite(Key, subregion, region, sep = ".", remove = FALSE)
# Join the 2 tibbles
state_join <- left_join(counties, report_09_28_2020, by = c("Key"))
# sum(is.na(state_join$Confirmed))
ggplot(data = us, mapping = aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) + 
  # Add data layer
...
ggplotly(
  ...
  coord_fixed(1.3) + 
# Add data layer
  geom_polygon(data = state_join, aes(fill = Confirmed), color = "red") +
    scale_fill_gradientn(colours = 
                         wes_palette("Zissou1", 100, type = "continuous")) +
...
###Exercise5
#Hello my fellow Floridian. Here is a quick report on the status of confirmed Covid cases in the U.S by counties.
```{r, echo=FALSE}
library(RColorBrewer)
#Hello my fellow Floridian. Here is a quick report on the status of confirmed Covid cases in the U.S by counties.
report_09_28_2020 <-   read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-28-2020.csv")) %>% 
...
#However, you must be pretty worried abut your own state. Therefore, I have included a gradient map of Florida divided by counties.
```
#However, you must be pretty worried abut your own state. Therefore, I have included a gradient map of Florida divided by counties.
...
#Finally, I have included an interactive map that lets you click on your home county to know the most recent updates in confirmed cases.
```{r, echo=FALSE}
...
#References: https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-28-2020.csv
#https://www.arcgis.com/apps/opsdashboard/index.html/bda7594740fd40299423467b48e9ecf6
#My github profile: https://npavo002.github.io/genomics-course
```
#References: https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-28-2020.csv
#https://www.arcgis.com/apps/opsdashboard/index.html/bda7594740fd40299423467b48e9ecf6
#My github profile: https://npavo002.github.io/genomics-course
